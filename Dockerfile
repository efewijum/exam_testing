...
RUN mkdir  $HOME/.vnc
RUN echo 'dockertest' | vncpasswd -f > $HOME/.vnc/passwd
RUN chmod 600 $HOME/.vnc/passwd
RUN echo 'whoami ' >>/dockertest.sh
RUN echo 'cd ' >>/dockertest.sh
RUN echo "su -l -c  'vncserver :2000 -geometry 1280x800' "  >>/dockertest.sh
